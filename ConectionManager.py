...
class ConectionManager:
    ...
    def getSocketSource(self):
        if self.inputInformation.isSimulation:
            socketSource = "wss://testnet.binance.vision/ws/{}@kline_{}".format(
                self.inputInformation.coinSymbol, self.inputInformation.timeInterval
            )
        else:
            socketSource = "wss://stream.binance.com:9443/ws/{}@kline_{}".format(
                self.inputInformation.coinSymbol, self.inputInformation.timeInterval
            )
        return socketSource
    ...
    def updateWallet(self):
        try:
            ...
            self.wallet = {
                "crypto": self.getCurrencyInWallet(firstCoinName, balances),
    ...
    def updateIndicators(self, newCandle):
        ...
        self.indicators.updateAllIndicators(
            self.closingPrices,
            self.marketData.lowestPrices,
            self.marketData.highestPrices,
        )
    ...
    def executeTradeLogic(self):
        instruction = self.strategy.execute(
            self.indicators, self.currentPrice, self.wallet
        )
    ...
    def pushBuyOrder(self, instruction):
        ...
        try:
            order = self.client.create_order(
                symbol=symbol, side=side, type="MARKET", quoteOrderQty=quantity
            )
