...
class MarketIndicators:
    def __init__(self, market):
        ...
        self.minimalLowestPrice = self.calculateRecentLowestPrice(
            self.market.lowestPrices
        )
        self.maximalHighestPrice = self.calculateRecentHighestPrice(
            self.market.highestPrices
        )
        self.stochastic = self.calculateStochastic(
            self.market.closingPrices,
            self.market.lowestPrices,
            self.market.highestPrices,
        )
        self.stochastic_7 = self.calculate_k_Stochastics(
            7,
            self.market.closingPrices,
            self.market.lowestPrices,
            self.market.highestPrices,
        )
        self.ATR = self.calculateATR(
            self.market.closingPrices,
            self.market.lowestPrices,
            self.market.highestPrices,
        )
    ...
    def updateAllIndicators(self, closePrices, lowestPrices, highestPrices):
        ...
        self.stochastic = self.calculateStochastic(
            closePrices, lowestPrices, highestPrices
        )
        self.stochastic_7 = self.calculate_k_Stochastics(
            7, closePrices, lowestPrices, highestPrices
        )
    ...
    def calculateMACD_Signal(self, closePrices):
        macdTrend = ta.trend.MACD(
            closePrices, window_fast=12, window_slow=26, window_sign=9
        )
        macd = macdTrend.macd()
    ...
    def calculateATR(self, closePrices, lowestPrices, highestPrices):
        ...
        atr = ta.volatility.AverageTrueRange(
            highestPrices, lowestPrices, closePrices, periodsConsidered
        ).average_true_range()
        return atr[-1]
    ...
    def calculate_k_Stochastics(self, k, closePrices, lowestPrices, highestPrices):
        stoch = ta.momentum.stoch_signal(
            highestPrices, lowestPrices, closePrices, window=14, smooth_window=3
        )
        return stoch[-k:]
